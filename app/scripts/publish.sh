...
for m in listening reading writing; do if [ -f public/data/$m/$id.json ]; then python3 scripts/validate_paper.py $m public/data/$m/$id.json | tail -1; else echo "(no ${m})"; fi; done
