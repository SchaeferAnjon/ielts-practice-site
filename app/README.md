...
| 科目 | 内容 | 来源 |
| --- | --- | --- |
| 听力 | 剑4-21 共 71 套（剑20 只有 T1-3 精简版）：每套 4 Part、40 题、答案、逐句原文（题号标记 + ASR 精确时间戳）、MP3（资料缺的段前端会标出） | 剑桥真题 PDF + 官方音频，清单见 `public/data/index.json` |
| 阅读 | 每套 3 篇、40 题、答案、每题定位句 + 中文解析 | 同上 |
| 写作 | 每套 Task 1（图表图片 + 可读出的数据表）+ Task 2，官方范文与考官评语 | 同上 |
| 口语 | 三季题库可切换：2026 年 5-8 月（当季）、2026 年 1-4 月、2025 年 9-12 月，带真实参考答案（英 + 中） | `scripts/extract_speaking.py` / `extract_speaking_qa.py` 从文字版 PDF 抽取 |
...
- **填 Key**：页面右上角「AI 设置」填接口地址、模型、Key（任何 OpenAI 兼容接口），存在浏览器 localStorage，不进构建包。本地开发也可以复制 `.env.example` 为 `.env` 当默认值。
- **公网部署**：不要把 API Key 写进 `.env` 再 build（会被打进前端包公开）。用 `proxy/cloudflare-worker.js` 部署一个带口令的代理，网站里接口地址填代理 URL、Key 填口令。接口在 `src/services/ai.ts`，返回结构与模拟完全一致。
